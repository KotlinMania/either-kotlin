...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Either.swiftmodule/`
...
//      `import Either` above would fail at compile time.
//
//   2. The static archive `libEither.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Either" }` and
//      `flattenPackage = "io.github.kotlinmania.either"` configuration in
...
final class EitherExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Either swift module imported cleanly")
    }
}